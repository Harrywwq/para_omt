; benchmark generated from python API
(set-info :status unknown)
(declare-fun strlen_72_64 () (_ BitVec 64))
(declare-fun args_0_8 () (_ BitVec 8))
(declare-fun strlen_74_64 () (_ BitVec 64))
(declare-fun args_1_8 () (_ BitVec 8))
(declare-fun strlen_77_64 () (_ BitVec 64))
(declare-fun args_2_8 () (_ BitVec 8))
(declare-fun strlen_81_64 () (_ BitVec 64))
(declare-fun args_3_8 () (_ BitVec 8))
(declare-fun sigaction_oldact_179_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_191_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_210_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_248_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_328_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_492_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_789_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_1449_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_2647_1216 () (_ BitVec 1216))
(assert
 (= (_ bv1 64) strlen_72_64))
(assert
 (let (($x69 (= (_ bv0 8) args_0_8)))
 (not $x69)))
(assert
 (= (_ bv1 64) strlen_74_64))
(assert
 (let (($x347 (= (_ bv0 8) args_1_8)))
 (not $x347)))
(assert
 (let (($x218 (= (_ bv1 64) strlen_77_64)))
 (not $x218)))
(assert
 (let ((?x192 (bvadd (_ bv576460752303357828 64) strlen_77_64)))
 (let (($x369 (= (_ bv0 8) args_2_8)))
 (let ((?x373 (ite $x369 (_ bv576460752303357828 64) (_ bv576460752303357829 64))))
 (= ?x373 ?x192)))))
(assert
 (let (($x415 (= (_ bv0 8) args_3_8)))
 (let ((?x225 (ite $x415 (_ bv576460752303357830 64) (_ bv576460752303357831 64))))
 (let ((?x197 (bvsub ?x225 (_ bv576460752303357830 64))))
 (= ?x197 strlen_81_64)))))
(assert
 (let ((?x2445 ((_ extract 127 120) sigaction_oldact_179_1216)))
 (let ((?x2507 (bvand ?x2445 (_ bv4 8))))
 (= (_ bv0 8) ?x2507))))
(assert
 (let ((?x2991 ((_ extract 127 120) sigaction_oldact_191_1216)))
 (let ((?x3035 (bvand ?x2991 (_ bv4 8))))
 (= (_ bv0 8) ?x3035))))
(assert
 (let ((?x4253 ((_ extract 127 120) sigaction_oldact_210_1216)))
 (let ((?x4273 (bvand ?x4253 (_ bv4 8))))
 (and (distinct (_ bv0 8) ?x4273) true))))
(assert
 (let ((?x6834 ((_ extract 127 120) sigaction_oldact_248_1216)))
 (let ((?x6911 (bvand ?x6834 (_ bv4 8))))
 (and (distinct (_ bv0 8) ?x6911) true))))
(assert
 (let ((?x12275 ((_ extract 127 120) sigaction_oldact_328_1216)))
 (let ((?x12281 (bvand ?x12275 (_ bv4 8))))
 (and (distinct (_ bv0 8) ?x12281) true))))
(assert
 (let ((?x24353 ((_ extract 127 120) sigaction_oldact_492_1216)))
 (let ((?x24370 (bvand ?x24353 (_ bv4 8))))
 (= (_ bv0 8) ?x24370))))
(assert
 (let ((?x45472 ((_ extract 127 120) sigaction_oldact_789_1216)))
 (let ((?x45478 (bvand ?x45472 (_ bv4 8))))
 (and (distinct (_ bv0 8) ?x45478) true))))
(assert
 (let ((?x83415 ((_ extract 127 120) sigaction_oldact_1449_1216)))
 (let ((?x83420 (bvand ?x83415 (_ bv4 8))))
 (= (_ bv0 8) ?x83420))))
(assert
 (let ((?x152214 ((_ extract 127 120) sigaction_oldact_2647_1216)))
 (let ((?x152234 (bvand ?x152214 (_ bv4 8))))
 (and (distinct (_ bv0 8) ?x152234) true))))
(maximize sigaction_oldact_492_1216)
(minimize sigaction_oldact_210_1216)
(maximize strlen_81_64)
(minimize strlen_74_64)
(maximize sigaction_oldact_789_1216)
(maximize sigaction_oldact_179_1216)
(minimize args_3_8)
(minimize sigaction_oldact_492_1216)
(minimize args_0_8)
(minimize sigaction_oldact_191_1216)
(maximize args_1_8)
(maximize sigaction_oldact_191_1216)
(maximize sigaction_oldact_210_1216)
(minimize sigaction_oldact_789_1216)
(maximize sigaction_oldact_1449_1216)
(minimize strlen_72_64)
(minimize sigaction_oldact_1449_1216)
(minimize strlen_81_64)
(maximize strlen_74_64)
(maximize sigaction_oldact_2647_1216)
(maximize args_0_8)
(minimize sigaction_oldact_2647_1216)
(maximize sigaction_oldact_248_1216)
(maximize args_2_8)
(maximize args_3_8)
(maximize strlen_77_64)
(minimize sigaction_oldact_179_1216)
(minimize sigaction_oldact_248_1216)
(minimize args_2_8)
(minimize sigaction_oldact_328_1216)
(maximize strlen_72_64)
(check-sat)
(get-objectives)
