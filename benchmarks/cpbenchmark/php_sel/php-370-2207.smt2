; benchmark generated from python API
(set-info :status unknown)
(declare-fun strlen_72_64 () (_ BitVec 64))
(declare-fun args_0_8 () (_ BitVec 8))
(declare-fun strlen_75_64 () (_ BitVec 64))
(declare-fun args_1_8 () (_ BitVec 8))
(declare-fun sigaction_oldact_171_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_175_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_184_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_198_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_223_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_271_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_375_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_557_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_900_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_1555_1216 () (_ BitVec 1216))
(declare-fun sigaction_oldact_2813_1216 () (_ BitVec 1216))
(assert
 (let (($x351 (= (_ bv1 64) strlen_72_64)))
 (not $x351)))
(assert
 (let ((?x82 (bvadd (_ bv576460752303357824 64) strlen_72_64)))
 (let (($x69 (= (_ bv0 8) args_0_8)))
 (let ((?x46 (ite $x69 (_ bv576460752303357824 64) (_ bv576460752303357825 64))))
 (= ?x46 ?x82)))))
(assert
 (let (($x347 (= (_ bv0 8) args_1_8)))
 (let ((?x355 (ite $x347 (_ bv576460752303357826 64) (_ bv576460752303357827 64))))
 (let ((?x356 (bvsub ?x355 (_ bv576460752303357826 64))))
 (= ?x356 strlen_75_64)))))
(assert
 (let ((?x1831 ((_ extract 127 120) sigaction_oldact_171_1216)))
 (let ((?x2319 (bvand ?x1831 (_ bv4 8))))
 (= (_ bv0 8) ?x2319))))
(assert
 (let ((?x2264 ((_ extract 127 120) sigaction_oldact_175_1216)))
 (let ((?x2349 (bvand ?x2264 (_ bv4 8))))
 (= (_ bv0 8) ?x2349))))
(assert
 (let ((?x2642 ((_ extract 127 120) sigaction_oldact_184_1216)))
 (let ((?x2659 (bvand ?x2642 (_ bv4 8))))
 (and (distinct (_ bv0 8) ?x2659) true))))
(assert
 (let ((?x3504 ((_ extract 127 120) sigaction_oldact_198_1216)))
 (let ((?x3529 (bvand ?x3504 (_ bv4 8))))
 (= (_ bv0 8) ?x3529))))
(assert
 (let ((?x4925 ((_ extract 127 120) sigaction_oldact_223_1216)))
 (let ((?x5144 (bvand ?x4925 (_ bv4 8))))
 (= (_ bv0 8) ?x5144))))
(assert
 (let ((?x8184 ((_ extract 127 120) sigaction_oldact_271_1216)))
 (let ((?x8190 (bvand ?x8184 (_ bv4 8))))
 (and (distinct (_ bv0 8) ?x8190) true))))
(assert
 (let ((?x15336 ((_ extract 127 120) sigaction_oldact_375_1216)))
 (let ((?x15361 (bvand ?x15336 (_ bv4 8))))
 (= (_ bv0 8) ?x15361))))
(assert
 (let ((?x28552 ((_ extract 127 120) sigaction_oldact_557_1216)))
 (let ((?x28577 (bvand ?x28552 (_ bv4 8))))
 (= (_ bv0 8) ?x28577))))
(assert
 (let ((?x51640 ((_ extract 127 120) sigaction_oldact_900_1216)))
 (let ((?x51652 (bvand ?x51640 (_ bv4 8))))
 (= (_ bv0 8) ?x51652))))
(assert
 (let ((?x88820 ((_ extract 127 120) sigaction_oldact_1555_1216)))
 (let ((?x88840 (bvand ?x88820 (_ bv4 8))))
 (= (_ bv0 8) ?x88840))))
(assert
 (let ((?x160352 ((_ extract 127 120) sigaction_oldact_2813_1216)))
 (let ((?x160368 (bvand ?x160352 (_ bv4 8))))
 (= (_ bv0 8) ?x160368))))
(maximize strlen_72_64)
(minimize strlen_72_64)
(maximize sigaction_oldact_900_1216)
(maximize sigaction_oldact_271_1216)
(minimize sigaction_oldact_2813_1216)
(minimize sigaction_oldact_375_1216)
(minimize sigaction_oldact_175_1216)
(minimize sigaction_oldact_900_1216)
(maximize sigaction_oldact_171_1216)
(minimize sigaction_oldact_1555_1216)
(minimize sigaction_oldact_557_1216)
(maximize args_1_8)
(maximize sigaction_oldact_175_1216)
(minimize sigaction_oldact_184_1216)
(minimize sigaction_oldact_198_1216)
(maximize strlen_75_64)
(minimize sigaction_oldact_271_1216)
(minimize args_0_8)
(maximize sigaction_oldact_375_1216)
(maximize sigaction_oldact_1555_1216)
(minimize sigaction_oldact_223_1216)
(maximize sigaction_oldact_184_1216)
(maximize args_0_8)
(minimize sigaction_oldact_171_1216)
(maximize sigaction_oldact_198_1216)
(minimize strlen_75_64)
(maximize sigaction_oldact_557_1216)
(maximize sigaction_oldact_223_1216)
(check-sat)
(get-objectives)
